...
- **主版本位**（5.x → 6.0.0）：配置文件格式或文件名变了、旧版插件读不了的大改动。6.0.0 就是这么来的：抽离过路径前缀的文件旧版读不了，所以连文件名一起换成了 `DirectoryV6.xml`。
...
- **新版平台的 EDT 不再隐式持有读锁**，Swing 监听器里直接碰 PSI 或索引会抛 `Read access is allowed from inside read-action only`（`ThreadingAssertions.assertReadAccess`）。2022.3 上不报，2024.1 起报——`NoteTreeView` 的双击跳转就是这么在 2026.2 上炸的（5.3.0 修）。补法是自己包一层读操作，用 `ApplicationManager.getApplication().runReadAction(Runnable)`：**不能用报错信息里推荐的 `WriteIntentReadAction`**（那个类 2024.1 才有，`since-build=223` 编不过），**也不能用 `ReadAction.run(ThrowableRunnable)`**（那个重载已废弃，Plugin Verifier 会报出来，5.3.2 换掉）。`TreesUtils.Navigation` 里 `findDirectory`/`findFile` 和 `selectPsiElement` 包在同一个 read action 里，因为后者内部还要再读一次 PSI 拿 `VirtualFile`。**要往外带值就用一元数组 + 语句块**：`runReadAction(() -> x[0] = f())` 写成表达式会在 `Runnable`、`Computable<T>`、`ThrowableComputable<T, E>` 三个重载之间歧义（报「对 runReadAction 的引用不明确」），必须写成 `runReadAction(() -> { x[0] = f(); })`。
  ...
  **判断一个回调在不在读锁下，看它是不是 `AnAction`**：平台只给 `AnAction.actionPerformed` 套那一层 write-intent read action，`JButton.addActionListener` 这种纯 Swing 回调什么都没有。5.7.5 修的就是这个——XML 工具窗口的「保存」按钮调到 `PsiManager.findFile` 直接炸，而同样调 `loadXmlFile` 的右键菜单一直好好的。补法是**在被调方自己包一层**（`XmlFileUtils.findXmlPsi`），每个调用点就不用各自记得包了；`XmlStorage.parsing` 还没有这层，从 Swing 回调调它要自己包（`XmlPrefixDialog.doSave` 就是这么做的）。**VFS 刷新不能一起包进去**：同步刷新自己要开写操作，套进读操作里是换一个错，所以 `refreshAndFindFileByIoFile` 一律留在读操作外面。

  沙箱验不了这条：2022.3 的 `PsiManagerImpl.findFile` 根本没有这个断言（实测后台线程 `isReadAccessAllowed()=false` 直接调也不抛），`ThreadingAssertions` / `EditorLockFreeTyping` 是 2024.1 之后才有的类。能验的是**锁语义本身**——在无锁线程上 `runReadAction` 前后打 `isReadAccessAllowed()`，从 `false` 变 `true` 就说明包对了，而新平台那个断言判的正是这个标志。想直接拿 2026.2 起沙箱是走不通的：`gradle-intellij-plugin 1.13.2` 用 2022.3.2 的 `util.jar` 拼 classpath 却跑在 2026.2 运行时上，`PathClassLoader` 不兼容，死在 `getSystemClassLoader cannot be called during the system class loader instantiation`。
...
- **列表里的 `AnAction` 一律不覆盖 `update()`**（5.5.0 起）。`getActionUpdateThread()` 的默认值 2022.3 是 EDT、2026.2 是 BGT，在 `update()` 里读 Swing 的选中状态跨版本不安全。所以这些 action 永远是启用的，空选中的情况在 `actionPerformed` 里处理（那里一定在 EDT 上），用 `Messages` 弹一句。删除类动作都要先 `Messages.showDialog(...)` 确认，**默认焦点放「取消」**（`defaultOptionIndex=1`），改的是用户项目里的 `DirectoryV6.xml`，不可逆。
...
- `XmlEditorToolWindow` 整个类 6.0.0 删了（连同 `plugin.xml` 里的 `TreeInfoTip Notes XML` 注册）。它的「抽离路径前缀」当时还是空 TODO，现在实现在 `PathPrefixes` / `PathPrefixEditor` 里、入口挪到了「目录备注」工具栏；「格式化」和「清理空属性」是正则实现、属性值里出现 `<` / `>` 时不安全，一并去掉了；「检查失效路径」和「目录备注」列表自带的标红重复，也去掉了。要在弹窗里改配置就用 `XmlPrefixDialog` 的「保存」。
